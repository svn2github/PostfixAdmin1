...
/**
 * @package PFA_Database
 * Core Database interface 
 * 
 * Thanks to Drupal for DBTNG
 * @copyright GPL by Drupal.org
 * @version $Revision: $:
 * @author $Author: $:
 * @date $Date: $:
 */
 ...
 * Every database driver implementation must provide a
...
abstract class DatabaseConnection extends PDO {
  ...
  function __construct($dsn, $username, $password, $driver_options = array()) {
    ...
    $this->setTableAliases(isset($this->connectionOptions['tables']) ? $this->connectionOptions['tables'] : '');
   ...
   *   default.settings.php.
   */
  protected function setTableAliases($table) {
    if (is_array($table)) {
      $this->prefixes = $prefix;
    }
  }
  ...
  public function prefixTables($sql) {
    ...
    foreach ($this->prefixes as $key => $val) {
      $sql = strtr($sql, array('{' . $key . '}' => $val . $this->tableAliases[$key]));
    }
...
abstract class Database {
   ...
   */
  final protected static function openConnection($target) {
    ...
    // error.
    if (!isset(self::$databaseInfo)) {
      throw new DatabaseConnectionNotDefinedException('The database connection is not defined.');
    }

    if (!$driver = self::$databaseInfo[$target]['driver']) {
      throw new DatabaseDriverNotSpecifiedException('Driver not specified for this database connection.');
    }

    // We cannot rely on the registry yet, because the registry requires an
    // open database connection.
    $driver_class = 'DatabaseConnection_' . $driver;
    require_once ROOT . '/lib/database/' . $driver . '/database.inc';
    ...
    if (!empty(self::$logs)) {
      $new_connection->setLogger(self::$logs);
    }
  ...
  final public static function parseConnectionInfo() {
    $databases = Conf::read('databases');
    
    $database_info = is_array($databases) ? $databases : array();
    foreach ($database_info as $target => $value) {
        // If there is no "driver" property, then we assume it's an array of
        // possible connections for this target. Pick one at random. That allows
        //  us to have, for example, multiple slave servers.
        if (empty($value['driver'])) {
          $database_info[$target] = $database_info[$target][mt_rand(0, count($database_info[$target]) - 1)];
        }

        // Parse the prefix information.
        if (!isset($database_info[$target]['prefix'])) {
          // Default to an empty prefix.
          $database_info[$target]['prefix'] = array(
            'default' => '',
          );
        }
        elseif (!is_array($database_info[$target]['prefix'])) {
          // Transform the flat form into an array form.
          $database_info[$target]['prefix'] = array(
            'default' => $database_info[$target]['prefix'],
          );
        }
        // Parse Table Aliases
        if (!isset($database_info[$target]['tables'])) {
        
          $database_info[$target]['tables'] = array(
            'default' => array(
              'admin' => 'admin',
              'alias' => 'alias',
    ...
    // database, user, and password keys in the same database array.
      ...
      foreach ($database_info as $target => $target_values) {
          self::$databaseInfo[$target] = $target_values;
      }
    }
  }
   ...
   */
  public static function addConnectionInfo($target, $info) {
    if (empty(self::$databaseInfo[$target])) {
      self::$databaseInfo[$target] = $info;
    }
  }
   ...
   */
  final public static function getConnectionInfo() {
    if (empty(self::$databaseInfo)) {
      self::parseConnectionInfo();
    }
   ...
   */
  public static function ignoreTarget($target) {
    self::$ignoreTargets[$target] = TRUE;
  }
