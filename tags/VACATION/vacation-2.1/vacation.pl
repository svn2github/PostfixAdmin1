...
sub do_cache {
   ...
   open (IN, "< cache.$to");
   @cache = <IN>;
   if (grep (/$from/, @cache)) { $in_cache = YES; }
   close (IN);
   if (!$in_cache) {
      open (OUT, ">> cache.$to") or die ("Unable to open cache file to write");
      print OUT "$from\n";
      close (OUT);
   }
   return $in_cache;
...
foreach $strip_element (@strip_to_array) {
   if ($strip_element =~ m/(\S*@\S*)/) { $strip_element = $1; }
   if ($strip_element =~ m/\<(.*)\>/) { $strip_element = $1; }
