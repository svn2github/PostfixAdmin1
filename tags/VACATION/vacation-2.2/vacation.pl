...
sub do_cache {
   my ($to, $from) = @_;
   $query = qq{SELECT cache FROM vacation WHERE email='$to'};
   $sth = do_query ($query);
   $row = $sth->fetchrow_array;
   @cache_array = split(/;/, $row);
   for ($i = 0; $i <= $#cache_array; $i++) {
      if ($cache_array[$i] =~ /^$from$/) { $in_cache = YES; }
   }
   if (!$in_cache) {
      $new_cache = "$row" . "$from" . ";";
      $query = qq{UPDATE vacation SET cache='$new_cache' WHERE email='$to'};
      $sth = do_query ($query);
   }
return $in_cache;
}
...
foreach $found_element (@found_array) {
   ...
   if ($rv == 1) {
      ...
      if (do_cache ($found_element, $from)) { next; }
